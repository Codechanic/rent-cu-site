...
namespace Vibalco\DatatableBundle\Util\Formatter;
    ...
    public function applyTo(array &$data)
    {
        foreach ($data as $row_index => $row)
        {
            ...
            foreach ($row as $column_index => $column)
            {
                ...
                {
                    $view = 'VibalcoDatatableBundle:Renderers:_default.html.twig';
                }
                $params = array_merge($params, array('dt_item' => $data[$row_index][$column_index],'dt_id'   => $identifier_raw)
                );
